...
// commit: 21618d410604e4720678b4311159227287ba05c2
contract UniswapV2 is IUniswapV2, ERC20("Uniswap V2", "UNI-V2", 18, 0) {
    ...
    function _update0(uint balance0, uint balance1) private {
        require(balance0 <= type(uint).max && balance1 <= type(uint).max, "UniswapV2: BALANCE_OVERFLOW");
    ...
    function mint0(address to) external lock returns (uint liquidity) {
    ...
    function burn0(address to) external lock returns (uint amount0, uint amount1) {
        uint balance0 = IERC20(token0).balanceOf(address(this));
        uint balance1 = IERC20(token1).balanceOf(address(this));
    ...
    function swap0(address tokenIn, uint amountOut, address to) external lock {
    ...
    function sync0() external lock {
        _update0(IERC20(token0).balanceOf(address(this)), IERC20(token1).balanceOf(address(this)));
    }
